...
translation **and** explicit syntactic parsing improve its ability to translate
...
text and (b) Basque dependency parsing; compare to a plain NLLB baseline.

## 2. Hypothesis

Exposing the decoder to explicit syntactic structure during fine-tuning reduces
structural hallucinations (spurious affixes, mangled ergative-absolutive
alignment) and improves translation of morphologically complex Basque output.
...
- Two tasks, single shared encoder-decoder, alternating batches:
  ...
  2. **Parsing:** Basque (`eus_Latn`) → linearised dependency parse on UD_Basque-BDT.
...
| parsing train | UD_Basque-BDT | `train` | ~5396 sentences |
| parsing dev | UD_Basque-BDT | `dev` | held out for parse-task monitoring |
...
- `src/data/ud_treebank.py` — Basque UD-BDT CONLLU loader that emits
  `{source: sentence, target: linearised_parse}` pairs. Parse target format is
  switchable: `"pos"`, `"deprel"`, or `"pos+deprel"` (default).
...
- `src/data/joint.py` — `JointMTLDataset` (reproducible interleaving of the
  two tasks with tunable `translate_weight`) and `build_joint_collator` (emits
  NLLB-ready batches with per-example `src_lang` and `forced_bos_token_id`).
...
- A training script (`src/train.py` or similar) that:
  - Constructs the joint dataset from `BasqueUDDataset` + `TatoebaEsEuDataset`.
  ...
  - Runs a joint-loss training loop: for each batch, compute seq2seq loss and
    optionally route per-task loss weighting. Labels already come masked with
    `-100` on padding so the default HF loss ignores them.
  - Checkpoints periodically; optionally phased curriculum à la Costa-jussà
    et al. 2022 to guard against overfitting on the Basque side.
...
3. Artetxe et al. 2020 — *Translation Artifacts in Cross-lingual Transfer Learning.* Relevant for interpreting eval-time behaviour when back-translated data is part of the training mix.
